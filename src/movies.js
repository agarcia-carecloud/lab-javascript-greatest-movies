...
function getAllDirectors(movieArr){
    const directors = movieArr.map(function(movie){
...
//Code shown by Mikayla Castro
function ratesAverage(movieArr){
    const totalRates = movieArr.reduce((accum, currValue) => {
      if(currValue.rate){
        accum+= currValue.rate;
      }
      return accum;
    },0);
    const rateAvg = totalRates/movieArr.length;
...
//Credit to Tomas Bennasar for recommending reusing function from Iteration 3
